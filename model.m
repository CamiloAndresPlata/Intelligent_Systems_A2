...
cd 'C:\Users\capla\OneDrive\Documents\IST\Intelligent systems\SI_Assignment_2'
wine_data = readtable('wine_data.txt');
cd ..
X = table2array(wine_data(:,2:14));
Y = table2array(wine_data(:,1));
rng(0,'twister');
...
%% Train initial 3-vs-all model
Y_train_3_vs_all = Y_train;
Y_train_3_vs_all(Y_train_3_vs_all==3) = -1;
Y_train_3_vs_all(Y_train_3_vs_all~=-1) = 0;
Y_train_3_vs_all(Y_train_3_vs_all==-1) = 1;

opt = genfisOptions('FCMClustering','FISType','sugeno');
opt.NumClusters = 3;
ts_model_3_vs_all = genfis(X_train,Y_train_3_vs_all,opt);
...
Y_pred_init_3_vs_all = evalfis(ts_model_3_vs_all, X_test);
Y_pred_init_3_vs_all(Y_pred_init_3_vs_all<0) = 0;
Y_pred_init_3_vs_all(Y_pred_init_3_vs_all>1) = 1;

Y_pred_init = zeros(size(Y_test,1),3);
Y_pred_init(:,1) = Y_pred_init_1_vs_all;
Y_pred_init(:,2) = Y_pred_init_2_vs_all;
Y_pred_init(:,3) = Y_pred_init_3_vs_all;
...
[in,out,~] = getTunableSettings(ts_model_1_vs_all);
...
[in,out,~] = getTunableSettings(ts_model_2_vs_all);
...
[in,out,~] = getTunableSettings(ts_model_3_vs_all);
anfis_model_3_vs_all = tunefis(ts_model_3_vs_all,[in;out],X_train,Y_train_3_vs_all,tunefisOptions("Method","anfis"));
...
Y_pred_final_3_vs_all = evalfis(anfis_model_3_vs_all, X_test);
Y_pred_final_3_vs_all(Y_pred_final_3_vs_all<0) = 0;
Y_pred_final_3_vs_all(Y_pred_final_3_vs_all>1) = 1;

Y_pred_final = zeros(size(Y_test,1),3);
Y_pred_final(:,1) = Y_pred_final_1_vs_all;
Y_pred_final(:,2) = Y_pred_final_2_vs_all;
Y_pred_final(:,3) = Y_pred_final_3_vs_all;
...
C = confusionmat(Y_test,Y_pred_final_labels);
confusionchart(C);
